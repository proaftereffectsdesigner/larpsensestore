        ...
        <div className="overflow-x-auto">
          <table className="w-full text-left border-collapse">
            ...
            <tbody className="divide-y divide-white/5">
              {filteredUsers.slice((usersPage - 1) * 10, usersPage * 10).map(user => (
          ...
          {filteredUsers.length > 10 && (
            <div className="p-4 border-t border-white/10 flex justify-center gap-2">
              <button onClick={() => setUsersPage(Math.max(1, usersPage - 1))} disabled={usersPage === 1} className="px-3 py-1 bg-white/5 hover:bg-white/10 rounded text-sm disabled:opacity-50 text-white font-bold transition-colors">Prev</button>
              <span className="text-gray-400 text-sm px-4 py-1 font-bold">Page {usersPage} of {Math.ceil(filteredUsers.length / 10)}</span>
              <button onClick={() => setUsersPage(usersPage + 1)} disabled={usersPage * 10 >= filteredUsers.length} className="px-3 py-1 bg-white/5 hover:bg-white/10 rounded text-sm disabled:opacity-50 text-white font-bold transition-colors">Next</button>
            </div>
          )}
        </div>
      </div>
