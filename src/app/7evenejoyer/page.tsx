    ...
    const fetchData = async () => {
      ...
      try {
        const [statsRes, usersRes, ticketsRes] = await Promise.all([
          ...
          fetch("/api/admin/tickets", { headers: { Authorization: `Bearer ${session.access_token}` } })
        ]);
      ...
      <div className="bg-[#111] border border-white/10 rounded-2xl overflow-hidden">
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-left border-collapse">
            ...
            <tbody className="divide-y divide-white/5">
              {tickets.filter(t => t.status === 'open').slice((ticketsPage - 1) * 10, ticketsPage * 10).map(ticket => (
          ...
          {tickets.filter(t => t.status === 'open').length > 10 && (
            <div className="p-4 border-t border-white/10 flex justify-center gap-2">
              <button onClick={() => setTicketsPage(Math.max(1, ticketsPage - 1))} disabled={ticketsPage === 1} className="px-3 py-1 bg-white/5 rounded text-sm disabled:opacity-50 text-white">Prev</button>
              <span className="text-gray-400 text-sm px-2 py-1">Page {ticketsPage}</span>
              <button onClick={() => setTicketsPage(ticketsPage + 1)} disabled={ticketsPage * 10 >= tickets.filter(t => t.status === 'open').length} className="px-3 py-1 bg-white/5 rounded text-sm disabled:opacity-50 text-white">Next</button>
            </div>
          )}
        </div>
      </div>
